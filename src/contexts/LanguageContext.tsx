...
import { createContext, useState, useEffect, ReactNode, useCallback, Suspense } from 'react';
...
export function LanguageProvider({ children }: { children: ReactNode }) {
  ...
  const [lang, setLangState] = useState<Language>(() => {
    ...
    return langParam === 'ta' ? 'ta' : 'en';
  });

  useEffect(() => {
    const langParam = searchParams.get('lang');
    if (langParam && (langParam === 'en' || langParam === 'ta') && langParam !== lang) {
